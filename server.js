...
// Below are message functions //
		...
		log('Message sent to room' +room+' by ' + username + 'success_data:'+JSON.stringify(success_data));
				...
				/* payload:
				   {
						 'requested_user': the socket id of person to be invited
					 }
					 invite_response:
					 	'result': 'success',
				...
				*/
						...
						if(!roomObject.sockets.hasOwnProperty(requested_user)){
							var error_message = 'invite requested a user that wasn\'t in room, command aborted';
							log(error_message);
							socket.emit('invite_response', {
																																		result: 'fail',
																																		message: error_message
					...
					// if everything is okay, respond to the inviter that it was successful //
						...
						socket.to(requested_user).emit('invited',success_data2);
					...
					/* payload:
						 {
							 'requested_user': the socket id of person to be uninvited
						 }
						 uninvite_response:
							'result': 'success',
				...
				// if everything is okay, respond to the uninviter that it was successful //
					...
					socket.to(requested_user).emit('uninvited',success_data2);
					log('uninvite successful');
									...
									/* payload:
										 {
											 'requested_user': the socket id of person to play with
										 }
										 game_start_response:
											'result': 'success',
									...
									*/
									...
									// Make sure the user being invited is in the room //
									if(!roomObject.sockets.hasOwnProperty(requested_user)){
										var error_message = 'game_start requested a user that wasn\'t in room, command aborted';
										log(error_message);
										socket.emit('game_start_response', {
																																					result: 'fail',
																																					message: error_message
								...
								// if everything is okay, respond to the game starter that it was successful //
									...
									socket.to(requested_user).emit('game_start_response',success_data2);
...
function create_new_game(){
	var new_game = {};
	new_game.player_cat = {};
	new_game.player_robot = {};
	new_game.player_cat.socket = '';
	new_game.player_cat.username = '';
	new_game.player_robot.socket = '';
	new_game.player_robot.username = '';
...
function send_game_update(socket, game_id, message){
	...
	/* Make sure that only 2 people are in game room */
	...
	do{
		...
		if(numClients > 2){
			...
			if(games[game_id].player_cat.socket == roomObject.sockets[0]){
				games[game_id].player_cat.socket = '';
				games[game_id].player_cat.username = '';
			}
			if(games[game_id].player_robot.socket == roomObject.sockets[0]){
				games[game_id].player_robot.socket = '';
				games[game_id].player_robot.username = '';
			}
	...
	if((games[game_id].player_cat.socket != socket.id) && (games[game_id].player_robot.socket != socket.id)){
		console.log('Player isn\'t assigned a color: '+socket.id);
		// and there isn't a color to give them //
		if((games[game_id].player_robot.socket != '') && (games[game_id].player_cat.socket != '')){
			games[game_id].player_cat.socket = '';
			games[game_id].player_cat.username = '';
			games[game_id].player_robot.socket = '';
			games[game_id].player_robot.uesrname = '';
		}
}

	/* Assign colors to players if not already done */
	if(games[game_id].player_cat.socket == ''){
		if(games[game_id].player_robot.socket != socket.id){
			games[game_id].player_cat.socket = socket.id;
			games[game_id].player_cat.username = players[socket.id].username;
		}
	}

	if(games[game_id].player_robot.socket == ''){
		if(games[game_id].player_cat.socket != socket.id){
			games[game_id].player_robot.socket = socket.id;
			games[game_id].player_robot.username = players[socket.id].username;
		}
	}
	...
	if(count == 64){
		...
		/* Delete old gamers after 1 hour */
